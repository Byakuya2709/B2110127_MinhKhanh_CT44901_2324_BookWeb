...
exports.confirmBorrow = async (req, res, next) => {
    try {
        const { borrowId } = req.body;

        const borrow = await BookManager.findById(borrowId);
        if (!borrow) throw new ApiError(404, "Mượn sách không tồn tại");
        ...
        res.status(201).json({ message: "Xác nhận mượn sách thành công" });
    } catch (error) {
        next(error);
    }
};